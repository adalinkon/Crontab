...
ENV VCPKG_ROOT=/usr/local/vcpkg \
    VCPKG_DOWNLOADS=${MOUNTPATH}/opt/vcpkg/downloads \
    VCPKG_DEFAULT_BINARY_CACHE=${MOUNTPATH}/opt/vcpkg/cache \
    XMAKE_GLOBALDIR=${MOUNTPATH}/opt \
    CONDA_DIR=${MOUNTPATH}/opt/linux_${TARGETPLATFORM}/miniforge3/
...
COPY .docker/base-scripts/*.sh /tmp/
RUN chmod +x /tmp/*.sh \ 
